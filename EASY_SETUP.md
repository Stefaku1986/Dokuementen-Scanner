...
5. Wenn du keine OneDrive- oder Kalender-Anbindung willst, stelle `graph.enable` auf `false`.
...
- Im Terminal (mit aktivierter Umgebung): `python -m document_scanner.cli run`.
...
- **OneDrive oder Kalender verwenden?** Lass einen Erwachsenen helfen, die Microsoft-Anmeldung zu machen (Device Code Flow). Danach kannst du `graph.enable` auf `true` setzen.

## 12. Service automatisch starten (optional)
- **Windows:** Suche nach "Aufgabenplanung" und erstelle eine Aufgabe, die beim Anmelden `python -m document_scanner.cli run` ausführt (mit aktiviertem `.venv`).
