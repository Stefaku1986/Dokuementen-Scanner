...
class DocumentProcessor:
    ...
    def upload(self, graph: GraphClient | None, local_path: Path, extracted: ExtractedData) -> Optional[str]:
        if not graph:
            logger.info("Graph deaktiviert, überspringe OneDrive-Upload")
            return None
        folder = (
    ...
    def create_calendar_event(
        self, graph: GraphClient | None, extracted: ExtractedData, onedrive_link: Optional[str]
    ) -> Optional[str]:
        if not graph:
            logger.info("Graph deaktiviert, kein Kalendertermin erforderlich")
            return None
        ...
        description = (
            ...
            f"OneDrive: {onedrive_link or 'lokal gespeichert'}\n" + "\n".join(extracted.summary)
        )
