...
- OneDrive-Upload und optionaler Outlook/Office 365 Kalendertermin über Microsoft Graph (Device Code Flow).
  Bei Bedarf kann Graph komplett deaktiviert werden (dann bleiben Dateien nur lokal).
...
4. **.env** (Beispiel):
   ```env
   OPENAI_API_KEY=sk-...
   # Nur falls Graph genutzt werden soll:
   GRAPH_CLIENT_ID=<Azure-App-ID>
   GRAPH_TENANT_ID=<Tenant-ID>
   ```
5. **config.yaml**: Von `config/config.example.yaml` kopieren und Pfade anpassen. Wenn Sie kein Microsoft Graph nutzen wollen, setzen Sie `graph.enabled: false`; die Felder `client_id`/`tenant_id` dürfen dann leer bleiben und Upload/Kalender werden übersprungen.
6. **Hotfolder**: Ordner aus `config` anlegen (`incoming`, `processed`, `failed`, `archive`).
7. **Microsoft Graph Registrieren (optional)**
...
6. Optional: Upload nach OneDrive `/Dokumente/<DocType>/<YYYY>/<MM>/` (nur wenn Graph aktiviert).
7. Optional: Falls Fälligkeitsdatum + Betrag vorhanden: Kalendertermin um 09:00 Uhr lokaler Zeit mit IBAN/Referenz/Link (nur wenn Graph aktiviert).
